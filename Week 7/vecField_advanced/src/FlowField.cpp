...
void FlowField::setMouse( const float &strength ) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set( ofGetMouseX() * strength, ofGetMouseY() * strength );
        }
    }
}

void FlowField::setSin( const float &strength ) {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set( sin(ofGetElapsedTimef()*TWO_PI) * strength, cos(ofGetElapsedTimef()*TWO_PI * strength) );
        }
    }
}
